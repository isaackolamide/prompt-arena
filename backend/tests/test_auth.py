...
def test_send_magic_link_failure(client: TestClient, mock_supabase):
    # Setup mock to raise AuthApiError
    mock_supabase.auth.sign_in_with_otp.side_effect = AuthApiError(
        "Failed to send OTP", 400, "otp_failed"
    )
...
def test_verify_otp_success(client: TestClient, mock_supabase):
    # Setup mock
...
def test_verify_otp_failure(client: TestClient, mock_supabase):
    # Setup mock to raise AuthApiError
    mock_supabase.auth.verify_otp.side_effect = AuthApiError(
        "Invalid OTP token", 401, "invalid_otp"
    )
    ...
    response = client.post(
        "/api/auth/verify",
        json={"email": "user@example.com", "token": "wrong-otp"},
    )
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert "Auth verification failed" in response.json()["detail"]


def test_verify_otp_no_session(client: TestClient, mock_supabase):
...
def test_send_magic_link_unexpected_error(client: TestClient, mock_supabase):
    # Setup mock to raise unexpected Exception
    mock_supabase.auth.sign_in_with_otp.side_effect = Exception(
        "Database is down"
    )
...
def test_verify_otp_unexpected_error(client: TestClient, mock_supabase):
    # Setup mock to raise unexpected Exception
...
def test_verify_otp_invalid_email(client: TestClient):
...
def test_magic_link_request(client: TestClient, mock_supabase):
    # Setup mock
    mock_supabase.auth.sign_in_with_otp.return_value = MagicMock()
...
def test_otp_verification(client: TestClient, mock_supabase):
    # Setup mock
    mock_auth_res = MagicMock()
    mock_session = MagicMock()
    mock_session.access_token = "mock-jwt-token"
